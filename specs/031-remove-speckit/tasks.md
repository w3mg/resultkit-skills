...
- [X] T014 Push branch, merge to main, and publish (`/ship-it`)
- [X] T015 [P] Verify: outside the repo, run `/plugin marketplace update` and confirm no `speckit:*` commands appear from the rkit plugin; also confirm all `rkit:*` skills load correctly (FR-003)
- [X] T016 [P] Verify: inside the repo, confirm `/speckit:specify` still works via `example-skills` plugin and `.specify/` directory is untouched

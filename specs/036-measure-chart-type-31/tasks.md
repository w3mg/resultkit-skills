...
- [x] T001 Update `api-reference.md` (master, repo root) to add `chart_type` to all 4 measure endpoints: add optional `chart_type?` to POST `/teams/{id}/measures` and PATCH `/measures/{id}` request bodies; add `chart_type` to GET `/teams/{id}/measures` and GET `/seats/{id}/measures` response shape notes; add validation note listing valid values and null semantics
...
- [x] T002 [US1] In `skills/scorecard/SKILL.md`, update the list view jq expression and table formatter to include a `chart_type` column; display the raw value when non-null and `—` when null; column should be rightmost in the measure table
- [x] T003 [P] [US1] In `skills/seats/SKILL.md`, update the "200: Show updated measures list" output block in the `align-measure` and `remove-measure` flows to include `chart_type` in the measures table (shown when non-null, omitted when null)
...
- [x] T006 Run `/sync-plugin` to copy `api-reference.md` to all `skills/*/references/api-reference.md` and bump the plugin patch version in `.claude-plugin/plugin.json`
...
**Sequential (same file — `skills/scorecard/SKILL.md`)**:
```
T002 → T004 → T005
```
Do these in order within a single session to avoid conflicts.

**Truly parallel (different file — `skills/seats/SKILL.md`)**:
```
T003  ← can run alongside any of T002/T004/T005
```

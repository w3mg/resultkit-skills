...
| Method | Path | Description | User Phrases | Web URL |
|--------|------|-------------|--------------|---------|
| GET | `/day-plans/today` | Today's plan (auto-creates if none exists). Priority-tag filter applied: items whose name contains a priority hashtag (`#next`, `#priority`, `#toppriority`, `#must`, `#1`–`#4`) are excluded when their status is not `active`. Item responses include `day_plan_action_id` (required when calling DPA-centric endpoints). | "show today", "my plan", "daily plan", "prioritizer" | `/day-plans/today` |

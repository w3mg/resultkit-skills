...
| Method | Path | Description | User Phrases | Web URL |
|--------|------|-------------|--------------|---------|
| GET | `/reviews` | List reviews (params: page, per_page, status, q, team_id). `team_id` filters directly by `group_id` on the review record — only reviews explicitly associated with that team are returned. Legacy reviews (null `group_id`) are excluded when team_id is provided. 400 if team_id invalid, 404 if not found. Omitting team_id returns all reviews. Excludes archived. Response includes `user_id` (creator ID, nullable), `team_id` (integer | null), `team` ({ id, name } | null). | "show reviews", "list reviews", "my reviews", "performance reviews" | — |
| POST | `/reviews` | Create review (body: reviewee_id*, reviewer_id*, template_id*, review_type?, start_date?, end_date?, team_id?). Admin/people-ops only. reviewee_id and reviewer_id must be members of at least one team in the account (400 "is not a member of any team in this account" if not). `team_id` is optional — when provided, must be a team in the same account. | "create review", "start review", "new performance review" | — |
| GET | `/reviews/{id}` | Review detail. Assessment visibility depends on requesting user's role. Response includes `user_id` (creator ID, nullable), `team_id` (integer | null), `team` ({ id, name } | null), `can_delete` (boolean — whether the authenticated user may delete this review), and `gwc_evaluations` (GwcEvaluationEntry[] — 0-2 entries, one per respondent_type). Reviewer GWC entries hidden from reviewee until `signed_off` status. | "show review", "review details", "open review" | — |
| PATCH | `/reviews/{id}` | Update review (body: review_type?, start_date?, end_date?, team_id?). Set `team_id` to an integer to associate, `null` to clear, omit to leave unchanged. | "update review", "change review dates", "edit review", "assign review to team" | — |
...
ReviewListItem fields: `id`, `user_id` (integer | null — creator ID), `reviewee` (UserSimple), `reviewer` (UserSimple), `status`, `review_type` (integer | null), `template` ({ id, name } | null), `team_id` (integer | null), `team` ({ id, name } | null), `start_date`, `end_date`, `created_at`.

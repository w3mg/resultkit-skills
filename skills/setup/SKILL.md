...
- api.sh: !`for p in "$HOME/.claude/plugins/"*/rkit/skills/setup/scripts/api.sh "$HOME/.claude/skills/rkit:setup/scripts/api.sh" "$HOME/.agents/skills/setup/scripts/api.sh" "$HOME/.gemini/skills/setup/scripts/api.sh" "scripts/api.sh"; do [ -f "$p" ] && echo "$p" && break; done || echo "NOT_FOUND"`

...
| POST | `/items` | Create item (body: name*, description, due, status, on_weekly, team_id, parent_id, context) | "add task", "create item", "new to-do", "add action item" |
| GET | `/items/{id}` | Get item detail (includes first-level children) | "show item", "item details", "open task", "what's in item X" |
| PATCH | `/items/{id}` | Update item (body: name, description, due, status, on_weekly) | "update item", "change status", "rename task", "set due date" |
| DELETE | `/items/{id}` | Archive item (soft delete, sets status=archived) | "archive item", "delete task", "remove item", "soft delete" |
| GET | `/items/{id}/children` | List child items (params: page, per_page, q) | "show sub-tasks", "list children", "nested items", "what's under this" |
| PUT | `/items/{id}/move` | Reposition item in tree (body: parent_id, left_id, right_id) | "move item", "reparent", "nest under", "reorder" |
...
| Method | Path | Description | User Phrases |
|--------|------|-------------|--------------|
| GET | `/items/{id}/assignees` | List assignees (paginated) | "who's assigned", "show assignees", "assigned to" |
| PUT | `/items/{id}/assignees` | Add assignee (body: user_id*) | "assign to", "add assignee", "give to" |
| DELETE | `/items/{id}/assignees/{user_id}` | Remove assignee | "unassign", "remove assignee", "take off" |

### Item Comments

| Method | Path | Description | User Phrases |
|--------|------|-------------|--------------|
| GET | `/items/{id}/comments` | List comments (chronological, paginated) | "show comments", "show notes", "what's been said" |
| POST | `/items/{id}/comments` | Create comment (body: body*) | "add comment", "leave a note", "comment on" |
...
| POST | `/teams` | Create team (body: name*, description, framework) | "create team", "new team", "add group" |
| GET | `/teams/{id}` | Get team detail (includes members) | "show team", "team details", "team info", "who's on the team" |
| PATCH | `/teams/{id}` | Update team (body: name, description, framework) | "update team", "rename team", "change framework" |
...
| Method | Path | Description | User Phrases |
|--------|------|-------------|--------------|
| GET | `/teams/{id}/members` | List members (params: page, per_page, q) | "show members", "who's on the team", "team roster" |
| PUT | `/teams/{id}/members` | Add member (body: user_id*, role?: "member"\|"admin") | "add member", "invite to team", "make admin" |
| DELETE | `/teams/{id}/members/{user_id}` | Remove member | "remove member", "kick from team" |

### Team Weekly Board (Items)

| Method | Path | Description | User Phrases |
|--------|------|-------------|--------------|
| GET | `/teams/{id}/items` | All team items (params: page, per_page, q, all) | "show weekly", "team board", "weekly board", "L10 board" |
| POST | `/teams/{id}/items` | Create team item (on_weekly=true) | "add to weekly", "new team task", "create on board" |
| PUT | `/teams/{id}/items/{item_id}` | Add item to board (sets on_weekly=true) | "put on weekly", "add to board", "show on weekly" |
...
| GET | `/teams/{id}/items/next` | Items with status=next (params: page, per_page, q, all) | "show next", "priorities", "to-dos (EOS)", "up next" |
| PUT | `/teams/{id}/items/next/{item_id}` | Add to board + set status=next | "move to next", "prioritize", "set as to-do" |
| GET | `/teams/{id}/items/done` | Items with status=done (params: page, per_page, q, all) | "show done", "completed", "finished items" |
| PUT | `/teams/{id}/items/done/{item_id}` | Add to board + set status=done | "mark done", "complete", "finish item" |
| GET | `/teams/{id}/items/issues` | Items with status=blocked (params: page, per_page, q) | "show issues", "blockers", "stuck items", "IDS (EOS)" |
| PUT | `/teams/{id}/items/issues/{item_id}` | Add to board + set status=blocked | "flag as blocked", "raise issue", "mark stuck" |
| GET | `/teams/{id}/items/parked` | Items with status=parked (params: page, per_page, q) | "show parked", "parking lot", "on hold", "deprioritized" |
| PUT | `/teams/{id}/items/parked/{item_id}` | Add to board + set status=parked | "park item", "put on hold", "deprioritize" |
...
| Method | Path | Description | User Phrases |
|--------|------|-------------|--------------|
| GET | `/teams/{id}/projects` | List active projects (params: page, per_page, q, all) | "show projects", "team projects", "rocks (EOS)", "execution plan" |
| POST | `/teams/{id}/projects` | Create project in team (body: name*, description, due, status, on_weekly, team_id, parent_id, context) | "create project", "new project on team", "add rock" |
| PUT | `/teams/{id}/projects/{project_id}` | Convert item to team project (sets type=TodoList, assigns to team). Idempotent. | "convert to project", "promote to project", "make it a project" |
| PATCH | `/teams/{id}/projects/{project_id}` | Update project (body: name, description, due, status, on_weekly) | "update project", "rename project", "change project status" |
...
| Method | Path | Description | User Phrases |
|--------|------|-------------|--------------|
| GET | `/users/me` | Authenticated user (includes api_token, default_team, current_team). Response wrapped in `{ data: { ... } }`. | "who am I", "my profile", "my token", "my API key" |
...
| GET | `/users/{id}` | User profile (no api_token). Returns UserPublic. | "show user", "user profile", "who is this" |
| GET | `/users/{id}/items` | User's items (requires same-team membership; params: page, per_page, q, status) | "show their tasks", "user's items", "what's assigned to them" |
...
| Method | Path | Description | User Phrases |
|--------|------|-------------|--------------|
| GET | `/day-plans/today` | Today's plan (auto-creates if none exists) | "show today", "my plan", "daily plan", "prioritizer" |
| GET | `/day-plans/today/items` | Today's items (params: page, per_page, q) | "today's tasks", "what's on today", "my plan items" |
| POST | `/day-plans/today/items` | Create item in today's plan (auto-creates plan) | "add to today", "new task for today", "put on my plan" |
| PUT | `/day-plans/today/items/{item_id}` | Attach existing item to today (auto-creates plan, body: position?) | "attach to today", "add to plan", "link to today" |
| PATCH | `/day-plans/today/items/{item_id}` | Toggle completion (body: completed*) | "check off", "mark done for today", "complete for today", "undo" |
| DELETE | `/day-plans/today/items/{item_id}` | Remove from plan (keeps item) | "remove from today", "take off plan", "drop from today" |
| GET | `/day-plans/{date}` | Plan by date (YYYY-MM-DD) | "show plan for Monday", "last Friday's plan" |
| GET | `/day-plans/{date}/items` | Items by date (params: page, per_page, q) | "items for that day", "what was on Monday" |
| POST | `/day-plans/{date}/items` | Create item in date's plan (plan must already exist) | "add to that day's plan" |
| PUT | `/day-plans/{date}/items/{item_id}` | Attach existing item to date (plan must already exist, body: position?) | "attach to that plan" |
| PATCH | `/day-plans/{date}/items/{item_id}` | Toggle completion (body: completed*) | "check off for that day" |
...
| GET | `/meetings/{id}` | Meeting detail (includes issues, done, next arrays) | "show meeting", "meeting details", "open meeting" |
| GET | `/meetings/{id}/items` | All meeting items (params: owner_id?, page, per_page, q) | "meeting items", "what's on the agenda" |
| POST | `/meetings/{id}/items` | Create item in meeting | "add to meeting", "new meeting item" |
| PUT | `/meetings/{id}/items/{item_id}` | Attach existing item | "attach to meeting", "link item to meeting" |
| DELETE | `/meetings/{id}/items/{item_id}` | Remove from meeting (keeps item) | "remove from meeting", "detach from meeting" |
| GET | `/meetings/{id}/items/next` | Next items (params: owner_id?, page, per_page, q) | "meeting next items", "meeting priorities" |
| GET | `/meetings/{id}/items/done` | Done items (params: owner_id?, page, per_page, q) | "meeting done items", "what got done" |
| GET | `/meetings/{id}/items/blocked` | Blocked items (params: owner_id?, page, per_page, q) | "meeting blockers", "meeting issues" |

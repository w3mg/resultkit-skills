...
- api.sh: !`for p in "$HOME/.claude/plugins/"*/rkit/skills/today/scripts/api.sh "$HOME/.claude/skills/rkit:today/scripts/api.sh" "$HOME/.agents/skills/today/scripts/api.sh" "$HOME/.gemini/skills/today/scripts/api.sh" "scripts/api.sh"; do [ -f "$p" ] && echo "$p" && break; done || echo "NOT_FOUND"`
- Today: !`date +%Y-%m-%d`
